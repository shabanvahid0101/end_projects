...
def change_word():
    ...
    # load random word from sqlite3 database
    conn = sqlite3.connect('D:/code/end_project/python/word game/vocabulary.db')
    c = conn.cursor()
    c.execute("SELECT * FROM vocabulary ORDER BY RANDOM() LIMIT 1")
    random_row = c.fetchone()
    conn.close()
    global random_kanji
    random_kanji=random_row[0]
    global random_hiragana
    random_hiragana=random_row[1]
    global random_persian
    random_persian=random_row[2]
    global random_english
    random_english=random_row[3]
    ...
    for i in range(3):
        conn = sqlite3.connect('D:/code/end_project/python/word game/vocabulary.db')
        d = conn.cursor()
        d.execute("SELECT * FROM vocabulary ORDER BY RANDOM() LIMIT 1")
        random_row2 = d.fetchone()
        random_hragana2=random_row2[1]
...
def time_countdown(count):
    ...
    if count > 0:
        time = count
        label_time.config(text=f"زمان باقی مانده:{time}")
        time -= 1
        root.after(1000, time_countdown, time)
    else:
        label_time.config(text=f"زمان تمام شد")
        btn1.config(state="disabled")
        btn2.config(state="disabled")
        btn3.config(state="disabled")
        btn4.config(state="disabled")
        next_btn.config(state="disabled")
        label_kanji.config(text="oops")
        label_status_persian.config(text="oops")
        label_status_english.config(text="oops")

#time_countdown(10)

#insert database
df = pd.read_csv("D:/file work/vocabulary-csv.csv")
...
title_up = tk.Label(canvas_title, text="آموزش لغات ژاپنی به صورت تخصصی", font=('Arial', 20), fg='blue')
...
#time label
label_time = tk.Label(canvas_time, text="زمان باقی مانده : ", font=('Arial',20))
label_time.place(x=10,y=10)

#kanji label
label_kanji=tk.Label(canvas_question,text="زمان را وارد کنید",font=("tahoma",20),fg="white",bg="brown")
...
next_btn = Button(master=canvas_DownButton, text="next", font=("tahoma,20"),fg="green",width=10,height=1,command=change_word)
next_btn.place(x=150,y=6)

#main function
def main():
    q = askinteger('question', 'insert time')
    change_word()
    time_countdown(q)
main()
root.mainloop()
